...
TEST(SerialDriverTest, InvalidDescriptorPathsAfterCommonInitAreReported) {
  ...
  EXPECT_EQ(serial_driver_write(1U, nullptr, 1U, &bytes),
            SERIAL_DRIVER_ERROR_INVALID_ARG);
  EXPECT_EQ(serial_driver_write(1U, reinterpret_cast<const uint8_t*>(""), 1U,
                                nullptr),
            SERIAL_DRIVER_ERROR_INVALID_ARG);
...
TEST(SerialDriverTest, GenericWriteCoversEdgeErrorPaths) {
  ...
  uint8_t data4[4] = {0x01U, 0x02U, 0x03U, 0x04U};
  uint8_t data1[1] = {0x05U};

  EXPECT_EQ(serial_driver_write(1U, data1, 1U, &bytes_written),
            SERIAL_DRIVER_ERROR_NOT_INITIALIZED);

  ASSERT_EQ(serial_driver_common_init(), SERIAL_DRIVER_OK);
  descriptor = serial_port_init(SERIAL_PORT_0, UART_PORT_MODE_SERIAL);
  ASSERT_NE(descriptor, SERIAL_DESCRIPTOR_INVALID);
  discrete_descriptor = serial_port_init(SERIAL_PORT_1, UART_PORT_MODE_DISCRETE);
  ASSERT_NE(discrete_descriptor, SERIAL_DESCRIPTOR_INVALID);

  EXPECT_EQ(serial_driver_write(discrete_descriptor, data1, 1U, &bytes_written),
  ...
  ASSERT_EQ(
      serial_driver_write(descriptor, data4, sizeof(data4), &bytes_written),
      SERIAL_DRIVER_OK);
  EXPECT_EQ(bytes_written, sizeof(data4));
  EXPECT_EQ(
      serial_driver_write(descriptor, data1, sizeof(data1), &bytes_written),
...
TEST(SerialDriverTest, GenericReadAndPendingCoverStagedAndErrorPaths) {
  ...
  EXPECT_EQ(serial_driver_read(1U, read_buf, 1U, &bytes_read),
            SERIAL_DRIVER_ERROR_NOT_INITIALIZED);

  ASSERT_EQ(serial_driver_common_init(), SERIAL_DRIVER_OK);
  discrete_descriptor = serial_port_init(SERIAL_PORT_1, UART_PORT_MODE_DISCRETE);
  ASSERT_NE(discrete_descriptor, SERIAL_DESCRIPTOR_INVALID);
  EXPECT_EQ(serial_driver_read(discrete_descriptor, read_buf, 1U, &bytes_read),
            SERIAL_DRIVER_ERROR_NOT_CONFIGURED);
  EXPECT_EQ(
      serial_driver_read(SERIAL_DESCRIPTOR_INVALID, read_buf, 1U, &bytes_read),
      SERIAL_DRIVER_ERROR_NOT_INITIALIZED);

  descriptor = serial_port_init(SERIAL_PORT_0, UART_PORT_MODE_SERIAL);
  ASSERT_NE(descriptor, SERIAL_DESCRIPTOR_INVALID);
  EXPECT_EQ(serial_driver_read(descriptor, read_buf, 1U, &bytes_read),
  ...
  ASSERT_EQ(
      ...
      SERIAL_DRIVER_OK);
  ASSERT_EQ(serial_driver_read(descriptor, read_buf, 2U, &bytes_read),
            SERIAL_DRIVER_OK);
  EXPECT_EQ(bytes_read, 2U);
  EXPECT_EQ(read_buf[0], 0xA1U);
  EXPECT_EQ(read_buf[1], 0xB2U);
...
TEST(SerialDriverTest, CoverageReachableDefensivePaths) {
  ...
  uint8_t data4[4] = {0x10U, 0x20U, 0x30U, 0x40U};
  ...
  EXPECT_EQ(serial_driver_write(1U, data4, sizeof(data4), &bytes_written),
            SERIAL_DRIVER_ERROR_NOT_INITIALIZED);
...
TEST(SerialDriverTest, InitFailsWhenDescriptorMapIsFull) {
  ...
  for (size_t i = 0U; i < UART_DEVICE_COUNT; ++i) {
    descriptors[i] = serial_port_init(static_cast<serial_ports_t>(i),
                                      UART_PORT_MODE_SERIAL);
...
TEST_P(SerialDriverPortTest, GenericReadSupportsPartialAndU32Alignment) {
  ...
  ASSERT_EQ(
      serial_driver_read(descriptor, read_buf, sizeof(read_buf), &bytes_read),
      SERIAL_DRIVER_OK);
  EXPECT_EQ(bytes_read, sizeof(read_buf));
  EXPECT_EQ(read_buf[0], 0xAAU);
  EXPECT_EQ(read_buf[1], 0xBBU);
  EXPECT_EQ(read_buf[2], 0xCCU);
